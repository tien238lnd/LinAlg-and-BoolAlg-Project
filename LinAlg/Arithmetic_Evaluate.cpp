		...
		if (tok.type == TOK_OPERATOR) {	// '*' '/' '+' '-' '~' '(' ')' 
			...
			default: // '*' '/' '+' '-' 
				while (!operators.empty() && priority(c) <= priority(operators.top())) {
					...
					if (ans_check != 0)
						return ans_check + 2; // 1+2 = 3: lỗi tính toán - 2+2 = 4: lỗi số chiều - 3+2 = 5: lỗi cú pháp - 4+2 = 6: lỗi chia cho 0
					operators.pop();
				}
...
Matrix evaluate_string_2_matrix(const string & str)
{
	...
	init(lex, str);
	int check_error = calculate_infix(tok, lex);
	if (check_error != 0)
		return Matrix();
	if (tok.type == TOK_MATRIX)
		return Matrix(*(Matrix*)(tok.valptr));
	if (tok.type == TOK_VECTOR)
		return Matrix(*(Vector*)(tok.valptr));
	else return Matrix();
}
...
{	// TOK_NUM double - TOK_VECTOR Vector - TOK_MATRIX Matrix
	...
	if (op == '~')
	{
		...
		{	// check lỗi số chiều
		case TOK_NUM:
			if (*(double*)newtok.valptr == DBL_MAX)
				return 2;
			break;
		case TOK_VECTOR:
			if (((Vector*)newtok.valptr)->size() == 0)
				return 2;
			break;
		case TOK_MATRIX:
			if (((Matrix*)newtok.valptr)->NRows() == 0)
				return 2;
			break;
		}
		operands.top() = newtok;
