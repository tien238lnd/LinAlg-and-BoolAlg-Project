...
void generate_evaluate2(VarsValue &vars_value, int n, const string &postfix_expr, vector<VarsValue> &minterms, unordered_map<char, int> &vars_idx)
{	// this function don't print truth table
	static int minterm_id = 0;
	if (n == vars_value.size()) {
	...
	vars_value[n] = 0;
	generate_evaluate2(vars_value, n + 1, postfix_expr, minterms, vars_idx);
	vars_value[n] = 1;
	generate_evaluate2(vars_value, n + 1, postfix_expr, minterms, vars_idx);
}
